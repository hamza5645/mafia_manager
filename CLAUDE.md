...
**Tech Stack**: SwiftUI + MVVM, iOS 18+, Tuist for project generation, Supabase Swift SDK
...
This project uses **Tuist** to manage the Xcode project. The `.xcodeproj` and `.xcworkspace` are generated — never edit them by hand.

**Key files:**
- `Project.swift` — project manifest (targets, dependencies, settings, Info.plist)
- `Tuist/Package.swift` — SPM dependency declarations
- `Tuist.swift` — Tuist configuration

**Workflow:**
```bash
# After adding/removing Swift files or changing dependencies:
tuist generate

# After changing SPM dependencies in Tuist/Package.swift:
tuist install && tuist generate

# Always open the workspace, not the .xcodeproj:
open mafia_manager.xcworkspace
```

**Adding new Swift files**: Just create the file anywhere under `App/`, `Core/`, or `Features/` and run `tuist generate`. Tuist auto-discovers all `.swift` files via the glob patterns in `Project.swift`. No manual Xcode project navigator work needed.

**Adding new resources**: Place files under `Resources/` or add a new glob pattern to the `resources:` array in `Project.swift`, then `tuist generate`.

**Generated artifacts** (gitignored):
- `Derived/` — generated Info.plists, Tuist helper files
- `mafia_manager.xcodeproj/` and `mafia_manager.xcworkspace/` — regenerated by `tuist generate`
...
# Or build via workspace (Tuist-generated)
xcodebuild -workspace mafia_manager.xcworkspace -scheme mafia_manager \
...
```bash
# Run all unit tests
xcodebuild -workspace mafia_manager.xcworkspace -scheme mafia_manager test \
...
### Cleaning
```bash
xcodebuild -workspace mafia_manager.xcworkspace -scheme mafia_manager clean
# For a full reset of DerivedData:
# rm -rf ~/Library/Developer/Xcode/DerivedData/mafia_manager-*
```
...
Run tests: `xcodebuild -workspace mafia_manager.xcworkspace -scheme mafia_manager test -destination "platform=iOS Simulator,name=iPhone 17 Pro"`
...
**Dependencies**: Supabase Swift SDK (declared in `Tuist/Package.swift`). Avoid adding new dependencies unless absolutely necessary.

## Essential Rules for iOS Work with Claude Code

1. **Never modify .pbxproj or .xcworkspace files** — These are generated by Tuist. To add files, just create them under `App/`, `Core/`, or `Features/` and run `tuist generate`. To change build settings or dependencies, edit `Project.swift` or `Tuist/Package.swift` and regenerate.
